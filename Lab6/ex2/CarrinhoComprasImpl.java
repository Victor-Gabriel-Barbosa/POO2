...
public class CarrinhoComprasImpl implements CarrinhoCompras {
  ...
  public void adicionaItem(Item item) {
    ...
    // Adiciona o novo item se não existir
    itens.add(item);
  }
  ...
  public double calculaFrete(FreteStrategy estrategiaFrete) {
    if (itens.isEmpty()) {
      System.out.println("Carrinho vazio, não há cálculo de frete.");
      return 0.0;
    }
    
    double valorCompra = calculaTotal();
    double valorFrete = estrategiaFrete.calcular(valorCompra);
    
    System.out.printf("Frete %s: R$ %.2f\n", estrategiaFrete.getNome(), valorFrete);
    return valorFrete;
  }
  ...
  public boolean realizaPagamento(PagamentoStrategy pagamentoStrategy) {
    ...
    if (valorTotal > 0) return pagamentoStrategy.pagar(valorTotal);
  ...
  public void mostrarItens() {
    ...
    for (Item item : itens) System.out.println(item);
    System.out.println("==========================");
